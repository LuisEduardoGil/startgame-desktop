...
async function sendGiftEmail({ to_email, order_id, gift_code, items, payment_method, total }) {
  ...
  const itemsHtml = (items||[]).map(i => {
    ...
    const imgTag = imgUrl ? `<img src="${imgUrl}" width="48" height="48" style="border-radius:10px;object-fit:contain;background:#1a1a2e;padding:4px;vertical-align:middle;margin-right:12px;" alt="${i.name}"/>` : "";
...
<body style="margin:0;padding:0;background:#0a0a0f;font-family:'Inter',sans-serif;">
  <div style="max-width:500px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:#111118;border-radius:24px 24px 0 0;padding:32px 28px 24px;text-align:center;border:1px solid rgba(255,255,255,0.08);border-bottom:none;">
      <img src="data:image/png;base64," width="72" height="72" style="border-radius:14px;margin-bottom:12px;display:block;margin-left:auto;margin-right:auto;" alt="Start Game"/>
      <p style="color:#ffffff;margin:0 0 2px;font-size:18px;font-weight:900;font-family:'Inter',sans-serif;letter-spacing:-0.3px;">Start Game</p>
    ...
    <div style="background:#111118;border:1px solid rgba(255,255,255,0.08);border-top:none;border-bottom:none;padding:32px 28px;">
      ...
      <!-- Check animado (SVG estático para email) -->
      <div style="text-align:center;margin-bottom:24px;">
        ...
        <h2 style="color:#ffffff;margin:0 0 6px;font-size:22px;font-weight:900;font-family:'Inter',sans-serif;letter-spacing:-0.5px;">Gift card entregada</h2>
        <p style="color:rgba(255,255,255,0.35);margin:0;font-size:13px;font-weight:500;font-family:'Inter',sans-serif;">Pedido <span style="color:#7B6FFF;font-weight:700;">#${order_id?.slice(0,8).toUpperCase()}</span></p>
      </div>
      ...
      <div style="background:rgba(0,200,150,0.06);border:1.5px solid rgba(0,200,150,0.2);border-radius:16px;padding:24px;text-align:center;margin-bottom:20px;">
        ...
        <div style="background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:14px 20px;margin-bottom:10px;">
          <p style="color:#ffffff;font-size:26px;font-weight:900;margin:0;letter-spacing:0.1em;font-family:'Courier New',monospace;">${gift_code}</p>
        </div>
        <p style="color:rgba(255,255,255,0.3);font-size:11px;margin:0;font-weight:500;font-family:'Inter',sans-serif;">Canjéalo en la plataforma correspondiente</p>
      </div>
      ...
      <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:18px;margin-bottom:20px;">
        ...
        <!-- Productos -->
        <div style="background:rgba(255,255,255,0.03);border-radius:10px;padding:4px 14px;margin-bottom:12px;">
          ${itemsHtml}
        </div>
        ...
        <table style="width:100%;border-collapse:collapse;">
          <tr>
            <td style="color:rgba(255,255,255,0.35);font-size:12px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.05);font-family:'Inter',sans-serif;font-weight:500;">Método de pago</td>
            <td style="color:#ffffff;font-size:12px;font-weight:700;text-align:right;padding:7px 0;border-bottom:1px solid rgba(255,255,255,0.05);font-family:'Inter',sans-serif;">${ML[payment_method]||payment_method}</td>
          </tr>
          <tr>
            <td style="color:rgba(255,255,255,0.35);font-size:12px;padding:7px 0;font-family:'Inter',sans-serif;font-weight:500;">Total pagado</td>
            <td style="color:#00C896;font-size:15px;font-weight:900;text-align:right;padding:7px 0;font-family:'Inter',sans-serif;">$${total}</td>
          </tr>
        </table>
      </div>
      ...
      <div style="background:rgba(37,211,102,0.05);border:1px solid rgba(37,211,102,0.15);border-radius:12px;padding:14px 16px;text-align:center;">
        <p style="color:rgba(255,255,255,0.4);font-size:12px;margin:0;font-family:'Inter',sans-serif;font-weight:500;line-height:1.6;">¿Algún problema con tu compra?<br/>Contáctanos por <span style="color:#25D366;font-weight:700;">WhatsApp</span> y te ayudamos.</p>
      </div>
    </div>
    ...
    <div style="background:#0d0d14;border:1px solid rgba(255,255,255,0.05);border-top:none;border-radius:0 0 20px 20px;padding:16px 28px;text-align:center;">
      <p style="color:rgba(255,255,255,0.15);font-size:10px;margin:0;font-family:'Inter',sans-serif;font-weight:500;">© 2025 Start Game &nbsp;·&nbsp; <a href="https://startgame.app" style="color:rgba(123,111,255,0.5);text-decoration:none;font-weight:600;">startgame.app</a></p>
    </div>
